/* Reasoning effort an agent ran at. Claude Code writes it on every assistant
   record as one of low | medium | high | xhigh | max; Codex CLI's scale adds
   minimal below low. The bars encode the level so the compact (bars-only)
   variant still reads at a glance. */
const EFFORT_RANK: Record<string, number> = { minimal: 1, low: 1, medium: 2, high: 3, xhigh: 4, max: 5 };
